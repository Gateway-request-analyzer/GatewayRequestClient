...
curl -X GET -H "userId: abc123" -H "session: null" -H "ip_address: 1.2.3.4" -H "RequestMethod: GET" -H "RequestURL: https://GRAProxy.com/catfact.ninja/breeds"    $GRA_GATEWAY/cat                                  # The user opens the app and login. This returns the session that will be used for all following requests.
sleep 0.2
